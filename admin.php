...
<head>
    ...
    <style>
        :root {
            --primary-bg: #f4f7f9; --sidebar-bg: #2c3e50; --header-bg: #ffffff;
            --primary-text: #34495e; --sidebar-text: #ecf0f1; --accent-color: #3498db;
            --white: #fff; --border-color: #e1e8ed; --shadow: 0 4px 12px rgba(0,0,0,0.08);
            --success: #27ae60; --danger: #c0392b; --warning: #f39c12; --info: #3498db;
        }
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar {
            ...
            position: fixed; top: 0; left: 0; height: 100%; display: flex; flex-direction: column; z-index: 100;
        }
        .sidebar-header {
            padding: 20px; text-align: center; font-size: 24px; font-weight: 700;
            border-bottom: 1px solid #34495e;
        }
        .sidebar-nav { flex-grow: 1; list-style: none; margin-top: 20px; }
        .sidebar-nav a {
        ...
        .header {
            background-color: var(--header-bg); height: 70px; display: flex;
            justify-content: space-between; align-items: center;
            padding: 0 30px; border-bottom: 1px solid var(--border-color);
        }
        #view-title { font-size: 22px; font-weight: 600; color: var(--primary-text); }
        .profile { position: relative; }
        .profile-btn { background: none; border: none; cursor: pointer; display: flex; align-items: center; gap: 10px; }
        .profile-btn span { font-weight: 600; }
        ...
        .card { background: var(--white); border-radius: 8px; padding: 25px; box-shadow: var(--shadow); margin-bottom: 25px; }
        
        /* Dashboard Cards */
        .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 25px; }
        .stat-card { background: var(--white); border-radius: 8px; padding: 20px; box-shadow: var(--shadow); border-left: 5px solid var(--accent-color); }
        .stat-card h3 { font-size: 16px; color: #7f8c8d; margin-bottom: 10px; font-weight: 500; }
        .stat-card p { font-size: 32px; font-weight: 700; }
        .stat-card.users { border-color: var(--info); }
        .stat-card.parses { border-color: var(--success); }
        .stat-card.orders { border-color: var(--warning); }
        ...
        .data-table th { background-color: #ecf0f1; font-size: 12px; text-transform: uppercase; }
        .data-table .actions button { border: none; color: var(--white); border-radius: 4px; cursor: pointer; margin-right: 5px; }
        
        .btn-view-details { background-color: var(--info); padding: 6px 12px; font-size: 13px; font-weight: 600; border-radius: 5px; cursor: pointer; transition: all 0.2s; white-space: nowrap; color: var(--white); border: none; }
        .btn-view-details:hover { transform: translateY(-1px); }
        ...
        .btn-primary { background-color: var(--accent-color); color: var(--white); border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; }
        .btn-secondary { background-color: #7f8c8d; color: var(--white); border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; }
        .btn-success { background-color: var(--success); color: var(--white); }
        .btn-danger { background-color: var(--danger); color: var(--white); }
        .btn-sm { padding: 5px 10px; font-size: 12px; }

        .login-container { width: 100%; height: 100vh; display: flex; justify-content: center; align-items: center; background: var(--primary-bg); }
        .login-panel { width: 100%; max-width: 900px; display: grid; grid-template-columns: 1fr 1fr; background: var(--white); border-radius: 12px; box-shadow: 0 10px 40px rgba(0,0,0,0.1); overflow: hidden; margin: 20px;}
        .login-form-section { padding: 50px; display: flex; flex-direction: column; justify-content: center; }
        .login-form-section h1 { font-size: 28px; margin-bottom: 10px; color: var(--sidebar-bg); }
        .login-form-section p { font-size: 16px; color: #7f8c8d; margin-bottom: 30px; }
        .message { text-align: center; padding: 10px; border-radius: 6px; margin-top: 15px; font-weight: 500; font-size: 14px; }
        .success { background-color: #e9f7ec; color: var(--success); }
        .error { background-color: #fbebee; color: var(--danger); }
        ...
        .modal-content { background-color: #fefefe; margin: 5% auto; padding: 25px; border-radius: 8px; width: 90%; max-width: 800px; position: relative; }
        ...
        .modal-header h2 { margin: 0; font-size: 20px; color: var(--primary-text); }
        .close-btn { color: #aaa; font-size: 28px; font-weight: bold; cursor: pointer; }
        


        @media (max-width: 900px) { .login-panel { grid-template-columns: 1fr; max-width: 450px; } .login-branding-section { display: none; } }
    </style>
</head>
<body>

    <div class="login-container" id="admin-login-view-wrapper" <?php if ($is_logged_in_as_admin) echo 'style="display:none;"'; ?>>
        <div class="login-panel">
            <div class="login-form-section">
                ...
                <button id="adminLoginBtn" class="btn-primary" style="width: 100%; padding: 15px;">Login Securely</button>
                <div id="admin-auth-message" class="message" style="display:none;"></div>
            </div>
            <div class="login-branding-section" style="background: linear-gradient(45deg, #34495e, #2c3e50); color: white; display:flex; flex-direction:column; justify-content:center; padding: 50px; text-align:center;">
                <h2>AiParcel</h2>
    ...
    <div class="dashboard-layout" id="app-view" <?php if (!$is_logged_in_as_admin) echo 'style="display:none;"'; ?>>
        <aside class="sidebar">
            <div class="sidebar-header">AiParcel</div>
            <ul class="sidebar-nav">
                <li><a href="#dashboard" class="nav-link active" data-title="Dashboard"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg><span>Dashboard</span></a></li>
                <li><a href="#users" class="nav-link" data-title="Users"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M15 21a6 6 0 00-9-5.197M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg><span>Users</span></a></li>
                <li><a href="#parses" class="nav-link" data-title="Parsed History"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg><span>Parsed History</span></a></li>
                <li><a href="#orders" class="nav-link" data-title="Order History"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10l2-2h8a1 1 0 001-1z"></path></svg><span>Order History</span></a></li>
                <li style="border-top: 1px solid #34495e; margin-top: 10px; padding-top: 10px;">
                    <a href="#plans" class="nav-link" data-title="Subscription Plans"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path></svg><span>Plans</span></a>
                </li>
                <li>
                    <a href="#payment-methods" class="nav-link" data-title="Payment Methods"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg><span>Payments</span></a>
                </li>
                <li>
                    <a href="#subscriptions" class="nav-link" data-title="Subscription Orders"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v.01"></path></svg><span>Subscriptions</span></a>
                </li>
                <li><a href="#settings" class="nav-link" data-title="Settings"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg><span>Settings</span></a></li>
            </ul>
        ...
        <main class="main-content">
            <header class="header">
                ...
                <div class="profile"><button class="profile-btn" id="profileBtn"><span id="adminDisplayName">Admin</span></button></div>
            </header>

            <div class="page-content">
                <div id="dashboard-view" class="view active"><div class="stats-grid"><div class="stat-card users"><h3>Total Users</h3><p id="total-users-stat">0</p></div><div class="stat-card parses"><h3>Total Parses</h3><p id="total-parses-stat">0</p></div><div class="stat-card orders"><h3>Total Orders</h3><p id="total-orders-stat">0</p></div></div></div>
                <div id="users-view" class="view"><div class="card"><div class="table-container"><table class="data-table display" id="users-table" style="width:100%"></table></div></div></div>
                <div id="parses-view" class="view"><div class="card"><div class="table-container"><table class="data-table display" id="parses-table" style="width:100%"></table></div></div></div>
                <div id="orders-view" class="view"><div class="card"><div class="table-container"><table class="data-table display" id="orders-table" style="width:100%"></table></div></div></div>
                    ...
                    <div class="grid-2">
                        <div class="card">
                            ...
                            <form id="plan-form">
                                ...
                                <div class="form-group"><label for="plan-name">Plan Name</label><input type="text" id="plan-name" required></div>
                                <div class="form-group"><label for="plan-price">Price (BDT)</label><input type="number" id="plan-price" step="0.01" required></div>
                                <div class="form-group"><label for="plan-limit-monthly">Monthly Order Limit</label><input type="number" id="plan-limit-monthly" placeholder="Leave blank for none"></div>
                                <div class="form-group"><label for="plan-limit-daily">Daily Order Limit</label><input type="number" id="plan-limit-daily" placeholder="Leave blank for none"></div>
                                <div class="form-group"><label for="plan-validity">Validity (Days)</label><input type="number" id="plan-validity" required></div>
                                <div class="form-group"><label for="plan-description">Description</label><textarea id="plan-description" rows="3"></textarea></div>
                                <div class="form-group" style="border-top: 1px solid var(--border-color); padding-top: 15px;">
                                    ...
                                    <div style="display: flex; flex-direction: column; gap: 10px; margin-top: 5px;">
                                        ...
                                        <label style="cursor:pointer; font-weight: normal; display:flex; align-items:center; gap: 8px;">
                                            <input type="checkbox" id="plan-can-correct-address"> Enable "Correct Address with AI"
                                        </label>
                                        <label style="cursor:pointer; font-weight: normal; display:flex; align-items:center; gap: 8px;">
                                            <input type="checkbox" id="plan-can-autocomplete"> Enable "Parse & Autocomplete"
                                        </label>
                                        <label style="cursor:pointer; font-weight: normal; display:flex; align-items:center; gap: 8px;">
                                ...
                                <div class="form-group"><label style="display:flex; align-items:center; gap: 8px; font-weight: normal;"><input type="checkbox" id="plan-is-active" checked> Active</label></div>
                        ...
                        <div class="card">
                            ...
                            <div class="table-container"><table id="plans-table" class="data-table display" style="width:100%"></table></div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid-2">
                        <div class="card">
                             ...
                             <form id="payment-method-form">
                                ...
                                <div class="form-group"><label for="payment-method-name">Method Name</label><input type="text" id="payment-method-name" required></div>
                                <div class="form-group"><label for="payment-method-details">Account Details</label><textarea id="payment-method-details" rows="3" required></textarea></div>
                                <div class="form-group"><label for="payment-method-instructions">Instructions</label><textarea id="payment-method-instructions" rows="3"></textarea></div>
                                <div class="form-group"><label style="display:flex; align-items:center; gap: 8px; font-weight: normal;"><input type="checkbox" id="payment-method-is-active" checked> Active</label></div>
                        ...
                        <div class="card">
                            ...
                            <div class="table-container"><table id="payment-methods-table" class="data-table display" style="width:100%"></table></div>
                        </div>
                    </div>
                </div>
                ...
                <div id="subscriptions-view" class="view">
                    <div class="card">
                        ...
                        <div class="table-container"><table id="subscriptions-table" class="data-table display" style="width:100%"></table></div>
                    </div>
                </div>
                        ...
                        <h2>Application Settings</h2>
                         <form id="settings-form">
                            ...
                            <div class="form-group">
                                ...
                                <img id="logoPreview" src="" alt="Logo Preview" style="max-height: 100px; margin-bottom: 10px; border: 1px solid var(--border-color); padding: 5px; border-radius: 5px; display: block;">
                                <input type="file" id="appLogoFile" accept="image/*">
                            </div>
                            <hr style="margin: 20px 0;">
                            <div class="form-group"><label for="geminiApiKey">Gemini API Key</label><textarea type="password" id="geminiApiKey"></textarea></div>
                            <div class="form-group"><label for="barikoiApiKey">Barikoi API Key</label><textarea type="password" id="barikoiApiKey"></textarea></div>
                            <div class="form-group"><label for="googleMapsApiKey">Google Maps API Key</label><input type="password" id="googleMapsApiKey"></div>
                            ...
                            <div class="form-group" style="border-top: 1px solid var(--border-color); padding-top: 20px;">
                                ...
                                <div style="display: flex; flex-direction: column; gap: 10px; margin-top: 5px;">
                                    <label style="cursor:pointer; font-weight: normal; display:flex; align-items:center; gap: 8px;"><input type="checkbox" id="showAiParseButton"> Show "Parse with AI" Button</label>
                                    <label style="cursor:pointer; font-weight: normal; display:flex; align-items:center; gap: 8px;"><input type="checkbox" id="showAutocompleteButton"> Show "Parse & Autocomplete" Button</label>
                                </div>
                            </div>
                            <div class="form-group" style="border-top: 1px solid var(--border-color); padding-top: 20px;">
                                <label for="helpContent">Help Modal Content (HTML/CSS Allowed)</label>
                                <textarea id="helpContent" rows="15" placeholder="Enter the HTML and CSS for your help guide here..."></textarea>
                            </div>
    ...
    <div id="user-details-modal" class="modal"> 
        <div class="modal-content">
            <div class="modal-header"><h2 id="user-details-title">User Details</h2><span class="close-btn">&times;</span></div>
    ...
    <div id="admin-profile-modal" class="modal"> 
        <div class="modal-content" style="max-width: 500px;">
            ...
            <div class="form-group">
                ...
                <button id="updateAdminNameBtn" class="btn-primary" style="width:100%; margin-top:10px;">Update Name</button>
            </div>
            <hr style="margin: 20px 0;">
            <div class="form-group">
                ...
                <button id="updateAdminPasswordBtn" class="btn-primary" style="width:100%; margin-top:10px;">Update Password</button>
            </div>
